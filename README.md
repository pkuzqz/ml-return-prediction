...
Features were then separated into three economically interpretable groups.
...
The signal was therefore clearly time-varying.
...
The quintile test therefore supports the existence of a volatility-related
...
The model included both regime indicators and interactions such as:

```math
x_j \cdot I(Z=\mathrm{mid\_vol})
```

and:

```math
x_j \cdot I(Z=\mathrm{high\_vol})
```

This allows feature coefficients to vary across regimes.
...
The backtest therefore does not disappear under moderately higher simplified
